...
const ScrollRevealText: React.FC<ScrollRevealTextProps> = ({
  ...
  return (
    <div 
      ref={containerRef}
      className="relative flex items-center justify-center min-h-[80vh] w-full overflow-hidden z-[5] p-8"
    >
      <h1 className="text-center leading-tight text-foreground m-0 p-0 md:text-[8rem]"
        style={{
          fontSize: fontSize,
        ...
        {words.map((word, wordIndex) => {
          ...
          return (
            <span key={wordIndex} className="inline-block mr-2 last:mr-0">
              ...
              {wordIndex < words.length - 1 && <span className="inline-block w-2"></span>}
            </span>
          );
        })}
      </h1>
